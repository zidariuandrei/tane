...
if (typeof Bun !== 'undefined') {
	...
	// Migration: Add model column if it doesn't exist (for existing DBs)
	try {
		db.run("ALTER TABLE seeds ADD COLUMN model TEXT DEFAULT 'gpt-4o'");
	} catch (_e) {
	...
	console.warn(
		'⚠️  Database mocked (Non-Bun environment detected). App functionality will be disabled.'
	);
}
